...
var (
	...
	defaultStasTime = 20 * time.Second
)
...
func (client *DockerClient) ChangeDefaultTime(timeOut, statsTime time.Duration) {
	defaultTimeout = timeOut
	defaultStasTime = statsTime
}

func (client *DockerClient) TOPContainer(id string) ([]*TopRes, error) {
	uri := fmt.Sprintf("/containers/%s/top?ps_args=aux", id)
	data, err := client.doRequest("GET", uri, nil, nil)
	if err != nil {
		return nil, err
	}
	var tops *TopConfig
	if err := json.Unmarshal(data, &tops); err != nil {
		return nil, err
	}

	res := []*TopRes{}
	for _, array := range tops.Processes {
		tem := &TopRes{}
		res = append(res, insert(tem, &array))
	}
	return res, err
}

func insert(res *TopRes, array *[]string) *TopRes {
	res.USER = (*array)[0]
	res.PID = (*array)[1]
	res.CPU = (*array)[2]
	res.MEM = (*array)[3]
